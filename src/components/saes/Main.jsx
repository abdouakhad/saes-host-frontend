    ...
    <>
          ...
          <div className=' lg:w-7/12 md:w-9/12 text-white md:px-10 px-5 md:mt-0 mt-24'>
            ...
            <div className='title mb-9 leading-loose font-bold md:pt-20'>
              <h1 className=' md:text-7xl text-5xl'>
                {' '}
                Logo & Website Design Agency
              </h1>
              <p className='tracking-widest mt-1 sm:text-3xl text-xl'>
                Service aux entreprises du Senegal
              </p>
            </div>
            <p className='font-light text-lg '>
              Nous sommes spécialisés dans la narration visuelle en créant des
              solutions cohérentes de conception de logos et de sites Web pour
              les entreprises, donnant des impressions durables au public dans
              un monde numérique.
            </p>
          </div>
        </div>
        ...
        <div className='flex space-x-6  flex-col justify-center items-center'>
          <div className='h-40 w-40 rounded-full ring-4 ring-bleu ring-offset-4 my-5 '>
            <img
              ...
              alt='Infographie Logo'
            />
          </div>
          <h1 className='font-extrabold text-bleu text-4xl md:text-6xl '>
            masterweb
          </h1>
        </div>
        <div className='tracking-wide mt-4 w-9/12 m-auto text-center text-xl'>
          Sed ut voluptatem accusantium doloremque laudantium, inventore
      ...
      <div className='section grid md:grid-cols-2 grid-cols-1 md:h-screen '>
        ...
        <div className=' web-section bg-my_gray md:py-5 py-10 lg:px-20 md:px-12 sm:px-8 px-4 md:flex md:flex-col  text-white  justify-center relative'>
          <div className='absolute img-code  md:right-16 right-28 top-20'>
            <img className='w-12' src={code} alt='code' />
          </div>
          <h1 className=' lg:text-6xl text-5xl'>
            <span className='text-bleu uppercase'>Design </span> EST
            <span className=' block uppercase'> stratégique.</span>
          </h1>
          <p className=' my-7 tracking-wide'>
            "Une stratégie de conception bien conçue produit systématiquement
            les résultats souhaités et la notoriété de la marque. Nous sommes
            convaincus que le succès réside dans une collaboration créative avec
            nos clients."
          </p>
          <p className='text-center mt-5'>
            <Link
              to='/masterweb/home'
              className='bg-bleu text-xl text-white  p-2 rounded-lg border-2 border-white hover:border-bleu hover:bg-white hover:text-bleu'
            >
              Visit Masterweb{' '}
            </Link>
          </p>
      ...
      <div className=' grid md:grid-cols-3 md:h-screen md:relative  md:mt-10'>
        <div className=' md:col-span-1 col-span-2  flex justify-center  md:bg-white bg-bleu md:text-my_gray text-white '>
          <p className='md:pt-28  md:w-4/12 w-9/12 md:-ml-5 md:absolute z-10 md:left-16 md:overflow-visible lg:text-6xl md:text-5xl text-5xl font-black md:mt-0 mt-10'>
            {' '}
            Approche pour créer une
            <span className='md:text-bleu text-my_gray border-b-8 border-bleu'>
              {' '}
              bonne{' '}
            </span>{' '}
            marque
      ...
      <div className='h-full flex flex-col justify-center items-center mb-10 md:mt-0 mt-10'>
        <div className='flex space-x-6 flex-col justify-center items-center'>
          ...
          <h1 className='font-extrabold text-jaune text-4xl md:text-6xl '>
            Infographie
          </h1>
        </div>
        <div className='tracking-wide mt-4 w-9/12 m-auto text-center text-my_gray text-xl'>
          Sed ut voluptatem accusantium doloremque laudantium, inventore
      ...
      <div className='grid md:grid-cols-3 relative md:h-screen bg-jaune'>
        ...
        <diV className='infoName col-span-1 '>
          <div className='md:rounded-br-3xl bg-jaune md:w-1/2 md:h-3/4 md:absolute left-0 px-10 flex justify-center items-center md:py-0 py-32 '>
            <h1 className=' font-black text-5xl text-my_gray '>
              Nom de marque & directives
              <p className='text-left mt-5 '>
                <Link
                  to='/infographie/home'
                  className='bg-my_gray text-xl text-jaune  p-2 rounded-lg border-2 border-jaune hover:border-my_gray hover:bg-gray-800 '
                >
                  Visit Infographie Sn{' '}
                </Link>
              </p>
            </h1>
          </div>
        </diV>
        <diV className='infoImg col-span-2 md:h-full md:rounded-bl-3xl md:h-full h-96'></diV>
      </div>
      ...
      <div className='infoApproach grid md:grid-cols-5 grid-cols-1 bg-my_gray '>
        <div className='md:col-span-2 bg-my_gray flex flex-col justify-center items-center md:py-0 py-10'>
          {approach_info.map((info) => (
            <Approach count={info.count} title={info.title} text={info.text} />
          ))}
        </div>
        <div className='col-span-3'>
          <img
            className='md:rounded-bl-3xl object-fill h-full'
            src={designTools}
            alt='design'
          />
        </div>
